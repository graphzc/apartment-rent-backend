...
export const RoomFindManySchema = Joi.object().keys({ where: Joi.object().keys(RoomWhereInputSchemaObject), orderBy: Joi.object().keys(RoomOrderByWithRelationInputSchemaObject), cursor: Joi.object().keys(RoomWhereUniqueInputSchemaObject), take: Joi.number(), skip: Joi.number(), distinct: Joi.array().items(RoomScalarFieldEnumSchema)  }).required()