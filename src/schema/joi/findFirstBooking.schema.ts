...
export const BookingFindFirstSchema = Joi.object().keys({ where: Joi.object().keys(BookingWhereInputSchemaObject), orderBy: Joi.object().keys(BookingOrderByWithRelationInputSchemaObject), cursor: Joi.object().keys(BookingWhereUniqueInputSchemaObject), take: Joi.number(), skip: Joi.number(), distinct: Joi.array().items(BookingScalarFieldEnumSchema) }).required()